...
use crate::types::{
    ...
    ProgressUpdate, YouTubeVideoInfo,
};
...
/// Generate document from YouTube video URL
...
where
    F: Fn(String),
{
    println!("🤖 [YOUTUBE] Starting YouTube video document generation");
    println!("📋 [YOUTUBE] URL: {}, Language: {}", youtube_video.url, language);
    emit_progress("YouTube動画からドキュメント生成を準備中...".to_string());
    ...
    let prompt = if let Some(custom) = custom_prompt {
        ...
        format!("Please analyze the YouTube video and create a comprehensive document based on the content. The document should include:
    ...
    emit_progress("Gemini AIにYouTube動画の分析を依頼中...".to_string());
    ...
    if response.status().is_success() {
        println!("✅ [YOUTUBE] Received successful response from Gemini API");
        emit_progress("AIの応答を受信中...".to_string());
        let gemini_response: GeminiResponse = response.json().await?;
        if let Some(candidate) = gemini_response.candidates.first() {
            if let Some(part) = candidate.content.parts.first() {
                if let GeminiPart::Text { text } = part {
                    println!(
                        "📝 [YOUTUBE] Generated document length: {} characters",
                        text.len()
                    );
                    emit_progress(format!("ドキュメント生成完了！ ({}文字)", text.len()));
                    return Ok(text.clone());
        ...
        println!("❌ [YOUTUBE] No text content found in response");
        emit_progress("エラー: AIの応答にテキストが含まれていません".to_string());
        Err(anyhow::anyhow!("No text content in response"))
    } else {
        let error_text = response.text().await?;
        println!("❌ [YOUTUBE] API request failed: {}", error_text);
        emit_progress(format!("エラー: AI生成に失敗しました - {}", error_text));
        Err(anyhow::anyhow!("API request failed: {}", error_text))
    }
}
